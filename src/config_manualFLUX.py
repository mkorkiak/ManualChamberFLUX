...
#Directory where fluxes are saved. 

#A folder called 'plots' is created into this folder, if plots are made
#The location needs to be in hyphens
#The folders can be separated by "/" (Linux and windows) or by "\" (Windows)
#If the folders are separated by "\", one needs to add "r" before the first hyphen like this:
...
#Include the gas mixing ratio columns below of which you want to calculate the fluxes 
#(these need to be DRY concentrations in PPM)
#You can also give headspace temperature, air pressure and PAR in this file. 
#Any other columns included in the data file will be added to the result file. So, include environmental values 
#such as RH, soil temperatures, etc. here if you want their closure time averages saved.
#If headspace temperature is given here AND also in the TIMES file, then the temperature from the TIMES file 
#will be used in the flux calculation.
    #CO2_dry (ppm)
    #CH4_dry (ppm)
    #N2O_dry (ppm)
    #CO_dry (ppm)
...
#Yes, it is enough to report the system height (no volume/surface area needed). HOWEVER, this assumes
#that the chamber shape is either cylindrical or rectangular (volume / area = height). 
#If, for some reason, you use a chamber of a different shape (please don't), be careful when calulcating the
#system height.

#If the data in the air pressure column is empty, the program will search for air pressure data in the chamber 
#data file. If no air pressure data is found in either of these two sources, the flux is calculated without 
#considering the air pressure.
#If the headspace temperature is specified here and also in the DATA file, then the T value from the TIMES file 
#will be used in the flux calculation.
...
#closure here if you want them saved with the respective fluxes.
#The location needs to be in hyphens.
#The folders can be separated by "/" (Linux and windows) or by "\" (Windows)
#But if the folders are separated by "\", one needs to add "r" before the first hyphen like this:
...
#If True, then starting and ending times in the TIMES file are used to plot the time series of the gases for 
#each closure. For each plot, the user must left-click the plot to select the start and end times. Selections 
#can be cancelled by right-clicking the plot. This program assumes that the first click indicates the starting 
#time, the second click indicates the ending time and the third click closes the plot.
#The selections are marked with red cross.
#It is recommended to click between the measurement points, rather than the points themselves, to ensure the 
#selected interval is what is wanted (all points located between the red crosses are included in the fitting).
...
#Save the new interactively selected starting and ending times in the new times file (True)

#The new TIMES file is saved in the folder indicated by the results_loc parameter below.
#The old times file is not replaced! The new file will have a suffix "_new" to avoid overwriting.
#If False, the interactively selected times will not be saved, but they will still be used in the flux 
#calculation.
...
#If set to 0, the starting time will be exactly the one selected interactively or the one given in the 
#times-file.
skip_start=0
...
#If the fit is longer than fit_maximum, datapoints from the end will be removed until the length is less than 
#fit_maximum
...
#If True, plot the mixing ratio time series of all the selected gases and the fits made into them and save the 
#figures.
plot_closures=True

#===============================================================================================================#

#Save also the fitting statistics (True) or just the fluxes and the optional columns in the data file (False)
...
#People are usually not interested in these, which is why there is an option not to save them to avoid bloating 
#the result file.
